...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	private boolean abstractFlag = false; //是否是抽象，对应的bean是否是abstract修饰

	private boolean lazyInit = false; //是否是懒加载

	private int autowireMode = AUTOWIRE_NO;

	private int dependencyCheck = DEPENDENCY_CHECK_NONE;//依赖项检查，spring3.0以后弃用

	@Nullable
	private String[] dependsOn;//一个bean的实例化依赖另一个bean的实例化，dependsOn为改bean属性名称集合

	//当autowireCandidate为false时，容器在自动装配过程中，该对象不会被考虑作为其他bean自动装配的候选者；
	//但是该bean本身还是可以使用自动装配来注入其他的bean
	private boolean autowireCandidate = true;

	private boolean primary = false;//当自动装配出现多个候选者时，primary为true时优选考虑
	...
	private boolean nonPublicAccessAllowed = true; //用来访问非公开的构造器和方法
	...
	//如果此时是factoryBean的话，该属性记录factoryMethodName
	@Nullable
	private String factoryMethodName;

	@Nullable
	private ConstructorArgumentValues constructorArgumentValues;//记录构造函数的参数属性
	...
	@Nullable
	private String initMethodName;//初始化方法，对于bean中的init-method方法

	@Nullable
	private String destroyMethodName;//bean销毁方法,对应bean中的destroy-method方法

	private boolean enforceInitMethod = true;//该属性控制是否执行init-method

	private boolean enforceDestroyMethod = true;//该属性控制是否执行destroy-method方法

	private boolean synthetic = false;//是否是用户定义的而不是程序定义的，创建AOP的时候为true
	...
	@Nullable
	private Resource resource;//bean的资源
