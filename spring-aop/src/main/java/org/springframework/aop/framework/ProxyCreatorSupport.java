...
public class ProxyCreatorSupport extends AdvisedSupport {
	 ...
	 */
	//1.该方法就是确定使用jdk代理还是cglib代理
	//2.最终返回一个AopProxy实例来创建代理对象；如果时jdk的代理会返回JdkDynamicAopProxy否则返回ObjenesisCglibAopProxy
	protected final synchronized AopProxy createAopProxy() {
		if (!this.active) {
			activate();
		}
		//1.获取创建代理对象的工厂AopProxyFactory，默认类型是DefaultAopProxyFactory
